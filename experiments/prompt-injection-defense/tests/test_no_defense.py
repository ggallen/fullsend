...
@patch("defenses.no_defense.anthropic")
def test_clean_commit_returns_not_detected(mock_anthropic):
...
@patch("defenses.no_defense.anthropic")
def test_injection_detected_via_format_deviation(mock_anthropic):
    mock_client = MagicMock()
    mock_response = MagicMock()
