...
- Runtime resolution: `fullsend run <name>` resolves agents in two tiers: (1) config entries from `OrgConfig.Agents` (highest priority), (2) runtime fallback to the `fullsend-ai/agents` repository for known first-party agents not in config. The fallback ref is controlled by `FULLSEND_UPSTREAM_REF` (set by the per-repo shim's `upstream_ref` input to match the installed version), falling back to `main` for dev builds or `v0` for releases (see [cli-internals.md](guides/dev/cli-internals.md)). The agents-repo fallback is a transitional mechanism for the agent extraction; it will be removed once all users have migrated to config-driven registration (ADR 0058 Phase 5).
